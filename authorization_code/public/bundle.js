...
// Toggle children.
				...
				$scope.get_metro_events = function(metro){
					...
					make_request_local(req).then(function(events){
						...
						events.forEach(function(event){
							...
							event.performance.forEach(function(perf){
								...
								if(match.distance >= .8){
									...
									artist_search_payload.push(perf);

									var req = {};
									//req.url = "https://api.spotify.com/v1/search?q=" + perf.displayName + "&type=artist";
							...
							$scope.search_artists(artist_search_payload);
						...
						// function requestAllWithDelay (actions, delay) {
						// 	return actions.reduce((promise, action) => {
						// 		return promise
						// 			.then((responses) => {
						// 				return action // Or whatever request library you're using. If it doesn't support promises, you can wrap it in `new Promise((resolve, reject) => someLib(url, { onSuccess: resolve, onError: reject }));` or something similar.
						// 					.then(response => {
						...
						// }
						//
						// requestAllWithDelay(promises, 700)
						//
						// 	.then(results => {
						//
						...
						// };
						//
						// // loop over uris and fetch each, waiting 1s between.
						...
						// 	}
						// }
						// doRequests(promises).then(function(results) {
						// 	console.log("$results", results);
						// });
				...
				$scope.playlist_tracks = function(user){
					return new Promise(function(done, fail) {
						...
						$http.post(url_local + payload.url_postfix,payload.body).then(function(res){
							...
							console.log("$playlistTrackMap",res.data);

							let artist_list = [];
							Object.keys(res.data).forEach(function(playlistId){
								artist_list = artist_list.concat($scope.process_tracks(res.data[playlistId],{id:playlistId},user))
							});
							...
							payloads.forEach(function(payload){
								...
								promises.push(make_request_simple(url_object, cache.dummy,300));
							});

							Promise.all(promises).then(function(results){

								$scope.process_artists(results,user);
				 ...
				 **/
				$scope.search_artists  = function(performances){
					...
					return new Promise(function(done, fail) {
						...
						req.type = "POST"
						req.body = {};
						req.body.performances = performances;
						let params =  getHashParams();
						req.body.token = params.access_token;
						req.url_postfix = "make_request_simple";
