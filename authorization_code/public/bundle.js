...
// Toggle children.
					...
					$scope.get_all_tracks = function(user){
						...
						console.log("getting tracks for " + $scope.user_cache[user.id]+ "'s  playlists..." );

						$scope.user_cache[user.id]['playlists'].forEach(function(playlist) {

							promiseTrack = promiseTrack.then(function() {
								return (
									$scope.playlist_tracks(user,playlist)
									//.then(sleeper(200))
								)
							});
						});
					...
					$scope.process_tracks = function(data,playlist,user){
					...
					$scope.playlist_tracks = function(user,playlist){
						...
						return new Promise(function(done, fail) {
							...
							let user_param = "/" + user.id;
							var url1 = "/playlists";
							var url2 = "/tracks";

							var url_object = {};
							url_object.url =  url_users + user_param + url1 + "/" + playlist.id + url2
							url_object.offset = 0;
							url_object.limit = 50;
							...
							make_request(url_object,cache.dummy)
								.then(function(data){
									...
									let artist_list = $scope.process_tracks(data,playlist,user);
